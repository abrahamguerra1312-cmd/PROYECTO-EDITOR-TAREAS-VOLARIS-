...
with st.sidebar:
    ...
    **Características:**
    ...
    - **Genera UN SOLO documento por archivo**
    """)
    ...
    add_blank_pages = st.checkbox(
        ...
        value=True,
        help="Añade una página en blanco si el documento termina en página impar"
    )
...
with st.expander("📖 Ejemplo de uso"):
    st.markdown("""
    **Archivo de entrada:** `XA-VLO.pdf` (40 páginas)
    **Archivo de salida:** `XA-VLO F.pdf` (25 páginas)
    
    **Mejoras aplicadas:**
    - Eliminación de hojas de control y metadatos
    - Eliminación de páginas con solo números
    - Eliminación de tablas vacías de control
    - **Generación de UN SOLO documento**
    - Añadida hoja en blanco al final si es necesario
    - Optimización para impresión a doble cara
    """)
    
    st.markdown("### 📋 Qué se elimina automáticamente:")
    st.markdown("""
    - Páginas con solo números (1, 2, 3, ...)
    - Páginas de metadatos (W/O Description, ATA, CMR, etc.)
    - Páginas de control de revisiones
    - Tablas de materiales vacías
    - Encabezados de sistema
    - Páginas de índice
    """)
